...
class YoutubeWidget(QWidget):
    ...
    def parse_youtube_link(self, video_url):
        if not video_url:
            ...
            self.messages('开始解析')
        self.parse_thread = ThreadParseQuality(vu=video_url)
        self.parse_thread.started.connect(
            self.start_parse_thread
        )
        self.parse_thread.video_urls_signal.connect(
            self.set_video_urls
        )
        self.parse_thread.audio_urls_signal.connect(
            self.set_audio_urls
        )
        self.parse_thread.title_signal.connect(
            self.set_title
        )
